...
export default function HomeScreen() {
  ...
  return <Screen>
    ...
    <LinearGradient colors={['#3366FF', '#2048D9']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.scanHero}>
      ...
      <Pressable onPress={() => router.push('/scan')} style={styles.scanButton}><View style={styles.scanIcon}><Ionicons name="barcode" size={30} color={colors.navy} /></View><Text style={styles.scanButtonText}>Scanner un produit</Text><Ionicons name="arrow-forward" size={20} color={colors.navy} /></Pressable>
...
const styles = StyleSheet.create({
  ...
  scanHero: { minHeight: 290, borderRadius: radius.xl, padding: 22, overflow: 'hidden', justifyContent: 'space-between' }, scanCopy: { maxWidth: 290, gap: 9, zIndex: 2 }, scanTitle: { color: colors.white, fontWeight: '900', fontSize: 28, lineHeight: 31, letterSpacing: -0.8 }, scanText: { color: '#E8EEFF', fontSize: 14, lineHeight: 20 }, scanButton: { height: 62, paddingHorizontal: 10, borderRadius: 19, backgroundColor: colors.sunshine, flexDirection: 'row', alignItems: 'center', gap: 11, zIndex: 2 }, scanIcon: { width: 44, height: 44, borderRadius: 14, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center' }, scanButtonText: { flex: 1, fontWeight: '900', color: colors.navy, fontSize: 16 }, orbitOne: { position: 'absolute', width: 180, height: 180, borderWidth: 35, borderColor: 'rgba(255,255,255,.09)', borderRadius: 90, right: -70, top: -40 }, orbitTwo: { position: 'absolute', width: 90, height: 90, borderWidth: 18, borderColor: 'rgba(255,255,0,.16)', borderRadius: 45, left: -35, bottom: 20 },
  ...
  drop: { flexDirection: 'row', alignItems: 'center', gap: 13 }, dropBadge: { width: 48, height: 48, borderRadius: 16, backgroundColor: colors.danger, alignItems: 'center', justifyContent: 'center' }, dropTitle: { fontSize: 16, fontWeight: '900', color: colors.ink }, dropText: { color: colors.muted, fontSize: 12, marginTop: 3 }, dropPrice: { fontSize: 20, fontWeight: '900', color: colors.danger }, disclaimer: { textAlign: 'center', color: colors.muted, fontSize: 11, lineHeight: 17, marginHorizontal: 18 },
});
